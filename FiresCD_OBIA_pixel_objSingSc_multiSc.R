...
# params$approach <- c("Pixel", "PixelMultiSc") 
...
params$ranger.vect <- c(50, 200)
params$spatialr.vect <- c(10)
...
params$multisc <- data.frame(ranger=c(200), spatialr=c(10), minsize=c(5))  ## parameters for multiscale: each row is a combination of the 3
...
#### START OPTIMIZATION OF MEANSHIFT PARAMS ON KEPT-IN FIRES --------------------------------------
  ...
  if ("ObjectSingleSc" %in% params$approach) {
    ...
    Fmeas.OOB.meanshift.dt <- data.table(matrix(nrow=0, ncol=4))   ## initialize empty matrix to store Out-of-Bag Overall Accuracies and associated parameters for each segmentation
    colnames(Fmeas.OOB.meanshift.dt) <- c("ranger", "spatialr", "minsize", "Fmeas")
    PA.OOB.meanshift.dt <- data.table(matrix(nrow=0, ncol=4))   ## initialize empty matrix to store Out-of-Bag Overall Accuracies and associated parameters for each segmentation
    colnames(PA.OOB.meanshift.dt) <- c("ranger", "spatialr", "minsize", "PA")
    UA.OOB.meanshift.dt <- data.table(matrix(nrow=0, ncol=4))   ## initialize empty matrix to store Out-of-Bag Overall Accuracies and associated parameters for each segmentation
    colnames(UA.OOB.meanshift.dt) <- c("ranger", "spatialr", "minsize", "UA")
...
#### SINGLE-SCALE RF TO GET OOB KAPPA -----------------------------------------------------------
        ...
        if ("ObjectSingleSc" %in% params$approach) {
          ...
          if (params$parallel.RF) {
            set.seed(params$seed)
            nr.clusters <- min(params$ntree, detectCores())
            tot.nrtrees <- params$ntree
            ...
            RF <- foreach (ntrees=c(rep(nrtrees.clust, nr.clusters-1), remaind), .combine=combine, .multicombine=T, .packages=c('randomForest', 'data.table')) %dopar% {
              randomForest(x=dataset.MS.optimiz[, params$obj.predictors, with=FALSE], y=target.vec,
          ...
          conf.matrix <- RF$confusion[1:length(params$mort.class.labels), 1:length(params$mort.class.labels)]
          OOB.Kappa <- cohen.kappa(conf.matrix)[[1]]
          ...
          PA <- diag(conf.matrix) / rowSums(conf.matrix)  ## Producer's accuracy, 1 - omission error, TP/(observed total)
          UA <- diag(conf.matrix) / colSums(conf.matrix)  ## User's accuracy, 1 - commission error, TP/(predicted total)
          Fmeas <- (2*PA*UA)/(PA+UA)
          Fmeas.OOB.meanshift.dt <- rbind( Fmeas.OOB.meanshift.dt, data.table(ranger, spatialr, minsize, Fmeas=Fmeas[2]) )   ## grow dt with results
          PA.OOB.meanshift.dt <- rbind( PA.OOB.meanshift.dt, data.table(ranger, spatialr, minsize, PA=PA[2]) )   ## grow dt with results
          UA.OOB.meanshift.dt <- rbind( UA.OOB.meanshift.dt, data.table(ranger, spatialr, minsize, UA=UA[2]) )   ## grow dt with results
  ...
  if ("ObjectSingleSc" %in% params$approach) {
    ...
    Fmeas.OOB.meanshift.sorted.dt <- arrange(Fmeas.OOB.meanshift.dt, desc(Fmeas))   ## sort df with results by decreasing Fmeas
    PA.OOB.meanshift.sorted.dt <- arrange(PA.OOB.meanshift.dt, desc(PA))   ## sort df with results by decreasing Fmeas
    UA.OOB.meanshift.sorted.dt <- arrange(UA.OOB.meanshift.dt, desc(UA))   ## sort df with results by decreasing Fmeas
