...
file_dirs = [fire_img_dir, gt_dir, maps_dir, segment_dir]  ## create list of directories to iterate on
...
for file_dir in file_dirs:  ## iterate over the directories
    ...
    if file_dir != segment_dir:  ## case for all the directories except that with segments
    ...
    else:  ## case for the directory with segments
        os.chdir(file_dir)   ## set current directory here
        for i, fire in enumerate(params['fires']):   ## get fire names and element indices at the same time
            string = params['best_scales'][i]   
            string = string.replace('rr', '_ranger')  ## replace string parts to match saved file
            string = string.replace('sr', '_spatial')
            string = string.replace('ms', '_minsize')
            search_key = '*'+fire+string+'*'+'.shp'  ## construct search key with variable parts replaced by *
            file_to_add = glob.glob(search_key)[0]  ## get all the files match the pattern, that is one file only
            layer = arcpy.mapping.Layer(os.path.join(file_dir, file_to_add))
